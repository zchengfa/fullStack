...
module.exports = function (app,port,prisma) {
    ...
    router.post('/upload', upload.single('chunk'), async (req, res) => {
        ...
        try {
            ...
            res.send({ code: 200, message: `分片 ${index} 上传成功` });
        } catch (error) {
    ...
    router.post('/mergeFile', async (req, res) => {
        ...
        if (!hash || totalChunks === undefined) {
            return res.status(400).send({ code: 400, error: '缺少必要参数: hash 或 totalChunks' });
        }
        ...
        try {
            ...
            try {
                ...
                return res.status(404).send({ code: 404, error: `未找到哈希值为 ${hash} 的分片目录` });
            }
            ...
            for (let i = 0; i < totalChunks; i++) {
                ...
                try {
                    ...
                    return res.status(400).send({ code: 400, error: `分片 ${i} 不存在` });
                }
            ...
            if (error.code === 'ENOENT') {
                res.status(404).send({ code: 404, error: '合并过程中文件或目录不存在' });
            } else {
                res.status(500).send({ code: 500, error: `服务器文件合并失败: ${error.message}` });
            }
        }
    });
    router.post('/checkFile', async (req, res) => {
        ...
        try {
            ...
            if (!hashArray || !Array.isArray(hashArray) || hashArray.length === 0) {
                return res.status(400).send({ code: 400, error: '缺少有效的文件哈希数组' })
            }

            if (!file_title) {
                return res.status(400).send({ code: 400, error: '缺少文件标题' })
            }
            ...
            if (!videoFile) {
                return res.status(400).send({ code: 400, error: '未找到视频文件' })
            }
