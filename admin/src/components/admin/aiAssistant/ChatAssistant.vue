...
import {throttle,URL} from '@/common/utils'
import {ref, onMounted, onBeforeUnmount, nextTick, PropType,watch} from "vue";
import SocketService from "@/socket/socket";
import {Right, Avatar, Clock, Close, FullScreen, Plus, Setting} from "@element-plus/icons-vue";
import ChatMessageList from "@/components/common/ChatMessageList.vue";
const assistantPinia = chatAssistantStore();
const userPinia = userStore();
const { isChatAssistant,mainMessageBody,isShowMask } = storeToRefs(assistantPinia);
...
defineProps({
  iconData:{
    ...
    default(){
      return [{icon: 'plus', message: '新建对话'},{icon: 'setting', message: '设置'},{icon: 'fullscreen', message: '窗口全屏'},{icon: 'history', message: '历史对话'}]
    }
  },
  maskQuestionList:{
    type: Array,
...
//关闭AI聊天组件
const closeChatAssistant = (e:any)=>{
  e.stopPropagation();
...
onMounted(()=>{
  ...
  socket.on('AIResults',(result:any)=>{
    ...
    //关闭等待回答状态
    assistantPinia.changeAnswerStatus(false)
  })
...
<template>
<div id="chat-assistant" :class="isChatAssistant ? 'chat-assistant-s' : 'chat-assistant-h'">
  <div class="assistant-top assistant-item">
    <div class="assistant-title">MM AI助理</div>
    <div class="icon-boxes">
      <div class="icon-box" v-for="(item,index) in iconData" :key="index">
  ...
  <div class="assistant-content assistant-item">
    <chat-message-list left-role="assistant" :message-data="mainMessageBody">
  ...
  <div class="assistant-bottom assistant-item">
    <div class="assistant-chat-box" :class="isChatBoxActive ? 'chat-box-active' : 'chat-box-normal'">
      ...
      <div class="chat-send">
        <div class="send-btn-box" @click="sendQuestion">
